...
#define STACK_SIZE_ETH_RX_LOOP 256

static TaskHandle_t rx_loop_freertos_handle;

static QueueHandle_t eth_rx_queue;

typedef struct {
    uint8_t  buf[CSP_ETH_BUF_SIZE];
    uint32_t len;
} eth_rx_frame;
...
void* csp_eth_rx_loop(void* param) {
    ...
    static eth_rx_frame frame;

    for (;;) {
        frame.len = (uint32_t)recvfrom(ctx->sockfd, frame.buf, CSP_ETH_BUF_SIZE, 0, NULL, NULL);

        if (frame.len <= 0)
            continue;

        BaseType_t should_context_switch = pdFALSE;
        xQueueSendFromISR(eth_rx_queue, &frame, &should_context_switch);
    }
}

void csp_eth_rx_loop_freertos(void* param) {
    ...
    while (1) {
        ...
        if (xQueueReceive(eth_rx_queue, &frame, portMAX_DELAY) != pdTRUE)
            continue;
        csp_eth_rx(&ctx->ifdata.iface, (csp_eth_header_t*)frame.buf, frame.len, NULL);
    }
...
int csp_eth_init(const char* device, const char* ifname, int mtu, unsigned int node_id, bool promisc, csp_iface_t** return_iface) {
    ...
    if ((ctx->sockfd = socket(AF_PACKET, SOCK_RAW, htobe16(CSP_ETH_TYPE_CSP))) == -1) {
        perror("socket");
        char exe[1024];
        int  count = (int)readlink("/proc/self/exe", exe, sizeof(exe));
        if (count > 0) {
    ...
    ctx->ifdata.tx_mtu = (uint16_t)mtu;

    static StaticQueue_t eth_rx_queue_qb;
    uint8_t              eth_rx_queue_buffer[10 * sizeof(eth_rx_frame)];

    eth_rx_queue = xQueueCreateStatic(10, sizeof(eth_rx_frame), eth_rx_queue_buffer, &eth_rx_queue_qb);

    /* Start server thread */
    static StaticTask_t csp_eth_rx_loop_tcb;
    static StackType_t  csp_eth_rx_loop_stack[STACK_SIZE_ETH_RX_LOOP];
    rx_loop_freertos_handle = xTaskCreateStatic(
        csp_eth_rx_loop_freertos,
        "CSP Rx loop",
        STACK_SIZE_ETH_RX_LOOP,
        ctx,
        tskIDLE_PRIORITY + 1,
